...
Three resolver layers (tried in order):
  1. Chroma lookup — fetch document text + stored metadata by chunk_id
  2. Evidence cache — match against chunks already retrieved for this session
  3. Chunk-ID heuristic — parse corpus_/sess_* IDs when lookup misses
"""
...
def resolve_citations(
    ...
    *,
    ...
    Returns {chunk_id: {chunk_id, found, source, source_label, section, page,
                        evidence_type, excerpt, filename}}.
    """
    ...
    for cid in unique_ids:
        if cid in chroma_hits:
            ...
            lookup[cid] = _normalize_entry(
                _resolve_from_chunk_id(cid),
                resolver="chunk_id_heuristic",
            )
    return lookup
...
def resolve_citation(
    ...
    *,
    ...
    """Resolve a single chunk_id (convenience wrapper)."""
...
# ---------------------------------------------------------------------------
# Resolver layer 2 — evidence cache (retrieved chunks from this run)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Resolver layer 3 — chunk_id heuristic
# ---------------------------------------------------------------------------
...
def _resolve_from_chunk_id(chunk_id: str) -> dict:
    ...
    return {
        ...
        "section": "",
        "page": _page_from_uploaded_id(chunk_id),
        "excerpt": "",
...
def _section_from_corpus_id(chunk_id: str) -> str:
    ...
    if not stem:
        return ""
    # Some stems embed article hints, e.g. corpus_article_5_chunk3 (future-proofing)
...
def _page_from_uploaded_id(chunk_id: str) -> str | None:
    return None
...
def _estimate_page_label(chunk_index: int | None) -> str | None:
    """Reserved for future PDF page metadata; not used as a fake page proxy."""
    return None
...
def _clean_chunk_text(text: str) -> str:
    """Strip markdown/HTML/table noise from converted corpus chunks."""
    text = text or ""
    text = re.sub(r"<[^>]+>", " ", text)
    text = re.sub(r"!\[[^\]]*\]\([^)]*\)", " ", text)
    text = re.sub(r"\[([^\]]+)\]\([^)]*\)", r"\1", text)
    # Markdown table rows and separators
    text = re.sub(r"^\s*\|[-:\s|]+\|\s*$", " ", text, flags=re.MULTILINE)
    text = re.sub(r"\|[-:\s|]+\|", " ", text)
    text = re.sub(r"\|+", " ", text)
    text = re.sub(r"\s*\|\s*", " ", text)
    # Recital / table cell markers like "(57) |"
...
def _make_excerpt(text: str, max_chars: int = 280, section: str = "") -> str:
    """
    Pick the most readable sentence from a chunk — not a blind prefix cut.

    Avoids mid-word starts, markdown table debris, and citation-only fragments.
    """
    ...
    # Prefer first substantive sentence in document order if top pick is junk
